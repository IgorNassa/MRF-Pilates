...
export async function inativarCliente(id: string) {
  try {
    const aulasFuturas = await prisma.appointment.findMany({
      where: { clientId: id, status: 'AGENDADO', date: { gte: new Date() } },
      include: { transaction: { select: { id: true } } }
    });

    const aulasDoPacote = aulasFuturas.filter(aula =>
      aula.type.toString().startsWith('PILATES_') && !aula.transaction
    ).length;
    ...
    await prisma.$transaction([
      prisma.client.update({
        ...
        data: {
          status: 'inativo',
          remainingSessions: { increment: aulasDoPacote }
        }
      }),
      prisma.appointment.updateMany({
        where: { id: { in: aulasFuturas.map(aula => aula.id) } },
    ...
    return { sucesso: false, erro: "Falha ao inativar cliente." };
  }
}
...
export async function ativarCliente(id: string) {
  try {
    ...
    return { sucesso: false, erro: "Falha ao ativar cliente." };
  }
}

// ==========================================
// 3. GESTÃO DE PLANOS & FINANCEIRO (BLINDADO)
// ==========================================
export async function atualizarPlanoCliente(clientId: string, novoPlano: string, parcelas: number, valorFinal: number, vencimento: number, paymentMethod: string, isPendente: boolean = false) {
  try {
    let meses = 1;
    if (novoPlano.toUpperCase().includes("TRIMESTRAL")) meses = 3;
    ...
    const isIsento = paymentMethod === 'ISENTO';
    const statusTransacao = isIsento ? 'PAGO' : (isPendente ? 'PENDENTE' : 'PAGO');
    const parcelasPagas = (statusTransacao === 'PAGO') ? 1 : 0;
    const valorReal = isIsento ? 0 : valorFinal;
    ...
    const futureAppts = await prisma.appointment.findMany({
      where: { 
        ...
        status: 'AGENDADO', 
        date: { gte: agora },
        type: { in: ['PILATES_1X', 'PILATES_2X', 'PILATES_3X', 'PILATES_5X'] }
      },
      orderBy: { date: 'asc' }
    });
    ...
    if (futureAppts.length > totalAulas) {
        const aulasParaManter = futureAppts.slice(0, totalAulas);
        const aulasParaCancelar = futureAppts.slice(totalAulas);

        if (aulasParaManter.length > 0) {
            await prisma.appointment.updateMany({
                where: { id: { in: aulasParaManter.map(a => a.id) } },
        ...
        if (aulasParaCancelar.length > 0) {
            await prisma.appointment.updateMany({
                where: { id: { in: aulasParaCancelar.map(a => a.id) } },
                data: { status: 'CANCELADO' }
            });
        }
        ...
        saldoCorrigido = totalAulas - futureAppts.length;
    }
    ...
    await prisma.client.update({
      ...
      data: { 
        plan: novoPlano, 
        planValue: valorReal, 
        planInstallments: parcelas, 
        planInstallmentsPaid: parcelasPagas, 
        planLastPayment: agora, 
        planDueDate: vencimento,
        planPaymentMethod: isIsento ? 'PIX' : paymentMethod, 
    ...
    if (valorReal >= 0) {
      ...
      await prisma.transaction.create({
        data: {
          ...
          paymentMethod: isIsento ? 'PIX' : paymentMethod as any,
          clientId: clientId,
          date: agora 
        }
      });
    ...
    console.error("ERRO CRÍTICO NO UPGRADE DE PLANO:", error);
...
export async function removerPlanoCliente(clientId: string) {
  try {
    const aulasParaCancelar = await prisma.appointment.findMany({
        where: {
            clientId,
            status: 'AGENDADO',
            type: { in: ['PILATES_1X', 'PILATES_2X', 'PILATES_3X', 'PILATES_5X'] },
...
export async function pagarParcelaPlano(clientId: string, valor: number, isento: boolean, isPendente: boolean = false) {
  try {
    ...
    const statusTransacao = isento ? 'PAGO' : (isPendente ? 'PENDENTE' : 'PAGO');
    let metodoSeguroDB = client.planPaymentMethod || 'PIX';
    if (isento) metodoSeguroDB = 'PIX'; 
    
    const novaParcelaPaga = (statusTransacao === 'PAGO') 
    ...
    await prisma.transaction.create({
      data: {
        ...
        type: 'RECEITA',
        category: 'MENSALIDADE',
        status: statusTransacao as any,
        paymentMethod: metodoSeguroDB as any,
        clientId: clientId,
        date: new Date()
      }
    })
...
export async function validarAgendamentosPreview(dados: any) {
  try {
    ...
    if (tipoAgendamento === 'REGULAR') {
      ...
      if (!clienteAtual || clienteAtual.status !== 'ativo') {
        return { sucesso: false, erro: 'Este aluno está inativo e não pode receber novos agendamentos.' };
      }
    }
    ...
    const data = appts.map(app => {
      ...
      if (app.type === 'EXPERIMENTAL') {
        ...
        if (app.client.planPaymentMethod === 'ISENTO' || app.client.planValue === 0) {
          ...
          let meses = 1;
          if (planStr.includes("TRIMESTRAL")) meses = 3;
